  ...
  useEffect(() => {
    const fetchPending = async () => {
      try {
        ...
        setPendingCount(data?.pending_approvals?.length || 0);
        setStats({
          pendingApprovals: data?.pending_approvals?.length || 0,
          activeCustomers: data?.branch_health?.reduce((sum, b) => sum + (b.active_customers || 0), 0) || 0,
        ...
        <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="mt-1 text-xs text-slate-600">
                            Transaction{pendingCount !== 1 ? 's' : ''} require{pendingCount === 1 ? 's' : ''} your review
                          </p>
                          <a
          ...
          <div className="relative" ref={profileRef}>
            ...
            >
              ...
              <span className="hidden sm:inline">{user?.name || user?.username || "Manager"}</span>
            </button>
                ...
                <div className="border-b border-slate-200 bg-gradient-to-br from-indigo-50 to-white p-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-indigo-600 text-lg font-bold text-white">
                      {(user?.name || user?.username)?.charAt(0).toUpperCase() || "M"}
                    </div>
                    <div>
                      <p className="font-semibold text-slate-900">{user?.name || user?.username || "Manager"}</p>
                      <p className="text-xs text-slate-500">{user?.email || "manager@example.com"}</p>
                      <p className="mt-1 text-xs font-medium text-indigo-600">{user?.role?.replace(/_/g, ' ').toUpperCase() || "MANAGER"}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="border-b border-slate-200 p-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-slate-900">{stats.pendingApprovals}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-600">Active Customers</span>
                        <span className="font-semibold text-slate-900">{stats.activeCustomers}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-600">High Risk Flags</span>
                        <span className="font-semibold text-rose-600">{stats.highRisk}</span>
                      </div>
                    </div>
                  </div>
                )}
